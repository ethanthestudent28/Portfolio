...
describe('Deleting a post when not authenticated', () => {
    before(() => {
...
describe('Deleting a post when authenticated', () => {
    before(() => {
        console.log('[Script: ' + filename + ']')
    })
    ...
    it('Should return a 404 if trying to delete the same post', () => {
        return chai.request(SERVER_URL)
            .delete('/posts/1')
            .set('X-Authorization', SESSION_TOKEN_USER1)
            .then((res) => {
